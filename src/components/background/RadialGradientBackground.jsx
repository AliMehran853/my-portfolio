...
function RadialGradientBackground({variant='hero', gradients=[]}) {
  ...
  const variants = {
    hero: [
      {
        position: 'top-1 -translate-x-1/2 -translate-y-1/2',
  ...
  const generalGradient = (color) => {
    const colorStops = color.map(({ color, stop }) => `${color} ${stop}`).join(', ');
  ...
  return (
    <div className='absolute inset-0 overflow-hidden pointer-events-none'>
      {activeGradients.map((gradient, index) => (
        <div key={index} className={`absolute ${gradient.position} ${gradient.size} rounded-full`} style={{ background: generalGradient(gradient.colors),
